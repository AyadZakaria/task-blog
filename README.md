# Tasks

# link to the hosted blog (task 1) : (https://tech-insights-ayad.vercel.app/)

![alt text](https://github.com/AyadZakaria/task-blog/blob/main/blog-preview.png?raw=true)

# link to the hosted and fixed form (task 2) : https://fixed-form-ayad.vercel.app/

![alt text](https://github.com/AyadZakaria/task-blog/blob/main/form-preview.png?raw=true)

# i have all the changes commented at the index.html (task 3)

# link to the hosted and fixed image dimensions (task 4) : https://fitted-image-ayad.vercel.app/

![alt text](https://github.com/AyadZakaria/task-blog/blob/main/fitted-image-preview.png?raw=true)

# link to the hosted pricing table (task 5) : https://pricing-table-ayad.vercel.app/

![alt text](https://github.com/AyadZakaria/task-blog/blob/main/pricing-table-preview.png?raw=true)

# link to the layout (task 6) : https://responsive-layout-ayad.vercel.app/

![alt text](https://github.com/AyadZakaria/task-blog/blob/main/responsive-layout-preview.png?raw=true)
